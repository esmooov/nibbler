...
export const addBits = (a: Nibble | number | null, b: Nibble | number | null): Nibble => {
...
export type Program = (nibbleA: Nibble, nibbleB: Nibble) => [Update, Update]
...
export const runNibblers = (program: Program): State => {
  ...
  return rows.reduce((state: State) => {
    const {nibbleA, NA, nibbleB, NB, history, isLooping, willStartLooping} = state
    ...
    const nextWillStartLooping = willStartLooping || history.some(({NA,NB}) => NA === nextNA && NB === nextNB) 
